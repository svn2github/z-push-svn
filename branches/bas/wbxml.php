...
class WBXMLDecoder {
	...
	function getElementStartTag($tag)
	{
		...
		if($element[EN_TYPE] == EN_TYPE_STARTTAG && $element[EN_TAG] == $tag)
			...
			debugLog('Unmatched tag '.(($element[EN_TYPE] == EN_TYPE_STARTTAG)?(isset($element[EN_TAG])?$element[EN_TAG]:'unknown'):'endtag').' != '.$tag );
			$this->ungetElement($element);
		}
	...
	function getElementEndTag()
	{
		...
		if($element[EN_TYPE] == EN_TYPE_ENDTAG)
			...
			debugLog("Unmatched end tag:");
			debugLog(print_r($element,true));
			$bt = debug_backtrace();
			$c = count($bt);
			debugLog(print_r($bt,true));
			debugLog("From " . $bt[$c-2]["file"] . ":" . $bt[$c-2]["line"]);
			$this->ungetElement($element);
		}

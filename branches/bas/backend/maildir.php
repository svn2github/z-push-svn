...
class BackendMaildir extends BackendDiff {
	...
	function BackendMaildir($config){
		$this->_config = $config;
	}
...
//		$folders[]=$sub;
	...
	function GetAttachmentData($attname) {
		list($id, $part) = unserialize(base64_decode($attname));
	...
	function GetMessage($folderid, $id, $truncsize) {
		...
		if(isset($message->parts)) {
			...
			if($part->ctype_primary == "application") {
					...
					$attachment->attname = base64_encode(serialize(array($id, $n)));
	...
	*/
	 ...
	 */
	function getPath() {
		return str_replace('%u', $this->_user, $this->_config['MIALDIR_DIR']);
	}
};

...
class ImportContentsChangesDiff extends DiffState {
	...
	if($id) {
			...
			if(strtolower(get_class($message)) == 'syncmail'){
	...
	function ImportMessageMove($id, $newfolder) {
		$change = array();
		$res = $this->_backend->MoveMessage($this->_folderid, $id, $newfolder);
		if($res == false)
			return false;
...
class ExportChangesDiff extends DiffState {
	...
	function GetChangeCount() {
		debugLog("diff::GetChangeCount() ". $this->_folderid);
...
class BackendDiff {
	...
	function Fetch($folderid, $id) {
		debugLog('diff::Fetch('.$folderid.', '.$id.')');
